...
class SOMFactory(object):
    ...
    def build(data,
        ...
        Constructs an object for SOM training, with the data parameters,
        ...
        Args:
            ...
            mapsize: tuple/list defining the size of the SOM map in the format (columns, 
                rows). For the development of periodicity in hexagonal 
...
class SOM(object):
    ...
    def training_summary(self):
        ...
        text_file.write(f"\n")
        text_file.write(f"Finetune Training:\n")
    ...
    def train(self,
        ...
        if self.training == 'batch':
            ...
            rough_qe_history = self.qe_history
            ...
            self.finetune_train(njob=n_job,
                                ...
                                train_len_factor=train_len_factor,
                                maxtrainlen=maxtrainlen)
            fine_qe_history = self.qe_history
            self.qe_history = np.array(rough_qe_history + fine_qe_history)
        ...
        if radius_plot:
            ...
            plt.plot(range(1, self.train_rough_len+1), np.linspace(self.train_rough_radiusin, self.train_rough_radiusfin, self.train_rough_len), marker="o", color="tab:red", label="Rough Training")
            plt.plot(range(self.train_rough_len+1,self.train_rough_len+self.train_finetune_len+1), np.linspace(self.train_finetune_radiusin, self.train_finetune_radiusfin, self.train_finetune_len), marker="o", color="tab:blue", label="Fine Tuning")
            plt.axvline(self.train_rough_len + 0.5, color="gray", linestyle="--", alpha=0.7)

            plt.legend()
            plt.xlabel("Iteration")
            plt.ylabel("Neighborhood Function Radius")
            plt.title("")
            plt.grid(True)
            plt.show()
        ...
        if qe_plot:
            ...
            plt.plot(range(1, self.train_rough_len+1), self.qe_history[:self.train_rough_len], 
                    marker="o", color="tab:red", label="Rough Training")
            plt.plot(range(self.train_rough_len+1,self.train_rough_len+self.train_finetune_len+1), self.qe_history[self.train_rough_len:], 
                    marker="o", color="tab:blue", label="Fine Tuning")
            plt.axvline(self.train_rough_len + 0.5, color="gray", linestyle="--", alpha=0.7)
    ...
    def _batchtrain(self,
        ...
        if self.missing == False:     
            ...
            for i in pbar:
                ...
                qe_history.append(self.QE)
            # end of the training loop for missing data
        ...
        # Training process for inputs with missing data
            ...
            for i in pbar:
                ...
                qe_history.append(self.QE)
            # end of the training loop without missing data
            self.params_json

        self.qe_history = qe_history
