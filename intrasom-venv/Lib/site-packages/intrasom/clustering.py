...
class ClusterFactory(object):
    ...
    def kmeans(self, k=3, init = "random", n_init=5, max_iter=200):
        ...
        kmeans = KMeans(n_clusters=k, init=init, n_init=n_init, max_iter=max_iter).fit(self.codebook)
        neurons_labels = kmeans.labels_+1 # get the neurons labels (from 1 to n_clusters)
        samples_labels = kmeans.labels_[self.bmus] # get the samples labels (from 0 to n_clusters-1) from its BMU
        clusters_centroids = kmeans.cluster_centers_ # (n_clusters, n_dims)

        self._neurons_labels = neurons_labels
        self._samples_labels = samples_labels
        self._clusters_centroids = clusters_centroids

        return neurons_labels.reshape(self.mapsize[1], self.mapsize[0])
    ...
    def plot_kmeans(self, 
        ...
        - title: str, optional
            Title of the created figure. Default: "Clustering Matrix".
    ...
    def plot_clusters_pca(
        ...
        # Load K-means neurons labels, samples labels and normalized centroids
        neurons_labels = self._neurons_labels
        max_clust = np.max(neurons_labels)
        # samples_labels = self._samples_labels
        clusters_centroids = self._clusters_centroids # normalized
