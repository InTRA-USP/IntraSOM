...
class PlotFactory(object):
    ...
    def __init__(self, som_object):
        ...
        self.lattice = som_object.lattice
        self.mapshape = som_object.mapshape
    ...
    def plot_umatrix(self,
        ...
        if resume:
            ...
            # n_rows = self.mapsize[1] = x
            # n_cols = self.mapsize[0] = 90 (gs_width = 100, legend_width = 3)
            plot_height = int(90*(self.mapsize[1]/self.mapsize[0]))
            gs_height = plot_height + 5
            gs = gridspec.GridSpec(gs_height, 100)
            ax = f.add_subplot(gs[:plot_height, 0:90])
            ax.set_aspect('equal')
            ...
            # Plot Parameters
            plt.xlim(-1.1, 2*self.mapsize[0]+0.1+0.5)
            plt.ylim(-0.5660254-0.6, 2*self.mapsize[1]*0.8660254-2*0.560254+0.6+0.2886751)
            ax.set_axis_off()
            plt.gca().invert_yaxis()
            ...
            # plt.subplots_adjust(bottom=3, top=5, hspace=0, wspace=0)
    ...
    def map_connections(self):
        """
        Find the pairs of neurons that are connected in the map using, by default, euclidean distances.
        
        Returns:
            connections: list of pairs of neurons.
        """

        mapsize = self.mapsize
        lattice = self.lattice
        mapshape = self.mapshape
        neurons = self.codebook
        distances = np.linalg.norm(neurons[:, np.newaxis, :] - neurons[np.newaxis, :, :], axis=-1)
        ...
        if lattice == 'hexa':
            ...
            if mapshape == 'planar':
                ...
                for i in range(mapsize[1]):
                    main_row = list(range(mapsize[0]*i+1,mapsize[0]*i+mapsize[0]+1))
                    lower_row = list(range(mapsize[0]*(i+1)+1,mapsize[0]*(i+1)+mapsize[0]+1))
                    # Get neighbors
                    for j in range(mapsize[0]):
                        # Lateral connections
                        if j == mapsize[0]-1: # last column
                            pass
                        else:
                            connections.append([main_row[j], main_row[j]+1])
                        # Main row-lower row connections
                        if i == mapsize[1]-1: # last row
                            pass
                        else:
                            if i%2 == 0: # even rows
                                if j == 0: # first column
                                    connections.append([main_row[j], lower_row[j]])
                                else:
                                    connections.append([main_row[j], lower_row[j]-1])
                                    connections.append([main_row[j], lower_row[j]])
                            else: # odd rows
                                if j == mapsize[0]-1: # last column
                                    connections.append([main_row[j], lower_row[j]])
                                    ...
                                    connections.append([main_row[j], lower_row[j]])
                                    connections.append([main_row[j], lower_row[j]+1])
            elif mapshape == 'toroid':
                # Iterate through each row
                for i in range(mapsize[1]):
                    main_row = list(range(mapsize[0]*i+1,mapsize[0]*i+mapsize[0]+1))
                    if i == mapsize[1]-1: # last row
                        lower_row = list(range(1,mapsize[0]+1))
                    else:
                        lower_row = list(range(mapsize[0]*(i+1)+1,mapsize[0]*(i+1)+mapsize[0]+1))
                    # Get neighbors
                    for j in range(mapsize[0]):
                        # Lateral connections
                        if j == mapsize[0]-1: # last column
                            connections.append([main_row[j], main_row[0]])
                        else:
                            connections.append([main_row[j], main_row[j]+1])
                        # Main row-lower row connections
                        if i%2 == 0: # even rows
                            if j == 0: # first column
                                connections.append([main_row[j], lower_row[j]])
                                connections.append([main_row[j], lower_row[-1]])
                            else:
                                connections.append([main_row[j], lower_row[j]-1])
                                connections.append([main_row[j], lower_row[j]])
                        else: # odd rows
                            if j == mapsize[0]-1: # last column
                                connections.append([main_row[j], lower_row[j]])
                                connections.append([main_row[j], lower_row[0]])
                            else:
                                connections.append([main_row[j], lower_row[j]])
                                connections.append([main_row[j], lower_row[j]+1])
        else:
            print("Non hexagonal lattice not implemented")
        
        return {(i, j): distances[i-1, j-1] for i, j in connections}
    ...
    def plot_som_pca(
                     ...
                     ylabel='PC 2'):
        """
        Function to visualize neurons and samples in the data space. The neurons weights and samples are projected into the first 2 Principal Component (PC). The PCA is fit into the normalized dataset and the neurons weights are just projected. The colors used for the neurons and connections are based on the euclidean distances between neurons, just like in the U-matrix.
        """
        # Load denormalized data
        if self.som_object.missing:
            data = self.som_object.imput_missing(save=False) # pandas dataframe
            data = data.values
        else:
            data = self.data_denorm

        # Load denormalized neuron weights matrix and the average distance of each neuron to its neighbors
        neurons = self.neuron_matrix
        u_dists = self.build_umatrix(expanded = False).reshape(-1, 1)

        # Create the connections object, the key contains the tuple of connected neurons and the values are obtained from the distance matrix between neurons
        connections = self.map_connections()

        # Normalize
        normalizer = NormalizerFactory.build(normalization)
        norm_data = normalizer.normalize(data)
        norm_neurons = normalizer.normalize(neurons)

        # Fit the PCA and project neurons weights
        pca = PCA(n_components=2, random_state=random_state)
        pca_data = pca.fit_transform(norm_data)
        self.pca = pca
        pca_neurons = pca.transform(norm_neurons)
